...
void    PING(User *user)
{
    ...
    if (params.size() == 0)
        return(user->get_server()->to_send(ERR_NOORIGIN(getArgs(), user->get_nick()), user->get_fd()));
    
    if (params[0] != user->get_server()->get_server_name())
        return(user->get_server()->to_send(ERR_NOORIGIN(getArgs(user->get_server()->get_address()), user->get_nick()), user->get_fd()));
