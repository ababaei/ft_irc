	...
	for (unsigned int i = 1; i < params[1].length(); i++)
	{
		if (params[1][i] == 'o' || params[1][i] == 'v' || params[1][i] == 'I' || params[1][i] == 'b')
		{
			...
			if (channel->isHere(params[2]) == false)
				return user->get_server()->to_send(ERR_USERNOTINCHANNEL(getArgs(2, params[2].c_str(),
								params[0].c_str()), user->get_nick()), user->get_fd());
			if (params[1][i] == 'o')
			{
				channel->setUserOp(params[2], b);
				if (channel->hasOneOp())
					user->get_server()->to_send(RPL_UNIQOPIS(getArgs(2, params[0].c_str(), params[2].c_str()),
		...
		else if (params[1][i] == 'i')
		{
			...
			user->get_server()->to_send(RPL_CHANNELMODEIS(getArgs(2, params[0].c_str(), sign + "i"),
						user->get_nick()), channel->getFds());
		}
		else if (params[1][i] == 's')
		{
			channel->setChanFlag("secret", b);
			user->get_server()->to_send(RPL_CHANNELMODEIS(getArgs(2, params[0].c_str(), sign + "s"),
						user->get_nick()), channel->getFds());
		}
		else if (params[1][i] == 'a')
		{
			channel->setChanFlag("anonymous", b);
			user->get_server()->to_send(RPL_CHANNELMODEIS(getArgs(2, params[0].c_str(), sign + "a"),
						user->get_nick()), channel->getFds());
		}
		else if (params[1][i] == 'm')
		{
			channel->setChanFlag("moderated", b);
			user->get_server()->to_send(RPL_CHANNELMODEIS(getArgs(2, params[0].c_str(), sign + "m"),
						user->get_nick()), channel->getFds());
		}
		else if (params[1][i] == 'l')
		{
			if (sign == "+")
			{
				...
				if (limit != 0)
				{
					...
					user->get_server()->to_send(RPL_CHANNELMODEIS(getArgs(3, params[0].c_str(), "+l", params[2]),
								user->get_nick()), channel->getFds());
				}
			}
			else
			{
				...
				user->get_server()->to_send(RPL_CHANNELMODEIS(getArgs(2, params[0].c_str(), "-l"),
							user->get_nick()), channel->getFds());
			}
		}
		else if (params[1][i] == 'k')
		{
			if (sign == "+")
			{
				...
				user->get_server()->to_send(RPL_CHANNELMODEIS(getArgs(3, params[0].c_str(), "+k", params[2]),
							user->get_nick()), channel->getFds());
			}
			else
			{
				channel->setKey("");
				user->get_server()->to_send(RPL_CHANNELMODEIS(getArgs(2, params[0].c_str(), "-k"),
							user->get_nick()), channel->getFds());
			}
		}
