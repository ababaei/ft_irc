...
/*   Updated: 2022/11/02 12:19:19 by amontaut         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	chanMsg(Channel* chan, User* user, std::vector<std::string>& params)
{
	if (chan->canSpeak(user->get_nick()) == false)
		return user->get_server()->to_send(ERR_CANNOTSENDTOCHAN(getArgs(1, params[0].c_str()), user->get_nick()), 
				user->get_fd());
...
void	PRIVMSG(User* user)
{
	...
	{
		Channel* channel = user->get_server()->get_channel(params[0].c_str());
		if (channel == NULL)
